...
from models_ddpm.data_utils import lattice_params_to_matrix_torch, get_pbc_distances, radius_graph_pbc, frac_to_cart_coords, repeat_blocks
...
class CSPNet(nn.Module):
    ...
    def __init__(
        ...
        self.atom_latent_emb = nn.Linear(hidden_dim + 256, hidden_dim)
        if act_fn == 'silu':
            self.act_fn = nn.SiLU()
        if dis_emb == 'sin':
    ...
    def forward(self, t, atom_types, frac_coords, lattices, num_atoms, node2graph):
        ...
        if self.ip:
            print(lattice_out.dtype)
            print(lattices.dtype)
