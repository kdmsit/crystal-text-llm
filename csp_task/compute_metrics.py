    ...
    def get_structure(self):
        if min(self.lengths.tolist()) < 0:
            ...
            try:
                ...
                self.structure = Structure(lattice=Lattice.from_parameters(*(length + angle)),
                                           species=self.atom_types,
                                           coords=frac_cord,
                                           coords_are_cartesian=False)
                self.constructed = True
            except Exception:
                print("Construction Exception")
                self.constructed = False
...
def get_crystal_array_list(file_path, batch_idx=0):
    ...
    if batch_idx == -1:
        ...
        if len(data['frac_coords'].size()) ==2:
            crys_array_list = get_crystals_list(data['frac_coords'], data['atom_types'], data['lengths'],
                                                data['angles'], data['num_atoms'])
        else:
            print(batch_idx)
            crys_array_list = get_crystals_list(data['frac_coords'][batch_idx],data['atom_types'][batch_idx],
                                            data['lengths'][batch_idx],data['angles'][batch_idx],data['num_atoms'][batch_idx])
